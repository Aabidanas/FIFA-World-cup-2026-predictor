...
def inject_global_css():
    ...
    :root {
        ...
        --accent: #FFFFFF; --accent-dim: #666666;
    ...
    h1 { font: 700 2rem/1.2 var(--font-display); color: var(--text-primary); letter-spacing: -0.03em; }
    h2 { font: 600 1.25rem/1.3 var(--font-display); color: var(--text-primary); letter-spacing: -0.02em; }
    h3 { font: 500 1rem/1.4 var(--font-display); color: var(--text-secondary); letter-spacing: 0.05em; text-transform: uppercase; font-size: 0.75rem; }
    ...
    div[data-testid="stButton"] button[kind="primary"] {
        background-color: var(--accent) !important; color: #000000 !important; border: none !important; border-radius: var(--radius-md) !important;
    ...
    div[data-testid="stButton"] button[kind="primary"]:hover { background-color: #E0E0E0 !important; transform: translateY(-1px) !important; }
    ...
    [data-testid="stProgress"] > div > div { background: linear-gradient(90deg, #FFFFFF, #666666) !important; border-radius: 99px !important; }
    ...
    .card-label { color: var(--text-muted); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 0.5rem; font-family: var(--font-mono); }
    </style>
...
def status_dot(color, label):
    colors = {"live": "#4CAF50", "idle": "#555", "error": "#CC3333"}
    st.markdown(f"""
...
def section_divider(label):
    ...
    <div style="display:flex;align-items:center;gap:1rem;margin:2rem 0">
      ...
      <span style="color:#444;font-size:0.7rem;letter-spacing:0.1em;text-transform:uppercase;font-family:var(--font-mono)">{label}</span>
      <div style="flex:1;height:1px;background:#1E1E1E"></div>
